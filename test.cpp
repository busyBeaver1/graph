...
int main(void) {
    ...
    g.reset_plecement(true); // сбрасываем положение графиков в окне на автоматически выбранное по умолчанию (чтобы все графики влезали)
    ...
    g.reset_plecement(true); // сбрасываем положение графиков с одинаковым масштабом по осям
    ...
    g.reset_plecement(-1.2, 1.2, -1.2, 1.2, true); // расположим систему координат так, чтобы квадрат [-1.2, 1.2]x[-1.2, 1.2] вмещался на экран
    ...
    g.reset_plecement(0.1, 4, 0.01, 100); // расположим систему координат так, чтобы квадрат [0.1, 4]x[0.01, 1000] вмещался на экран
