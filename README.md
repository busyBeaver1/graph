...
Скачайте из этого репозитория файл `graph.hpp` и файл шрифта `DejaVuSans.ttf` из [этого архива](http://sourceforge.net/projects/dejavu/files/dejavu/2.37/dejavu-sans-ttf-2.37.zip) и поместите их в папку проекта. `DejaVuSans.ttf` должен находиться в той папке, в которой скомпилированный код запускается (обычно это папка проекта).
...
- `bool logscale_н` - использовать ли логарифмическую шкалу по вертикальной оси.
